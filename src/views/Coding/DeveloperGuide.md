...
- `CodeEditor`: renders the header (engine toggle, Format, `EditorSettings`, `LanguagePicker`) and either `MonacoEditor` or `CodeMirrorEditor` based on `utils/editorSettingsStore.ts`'s persisted engine choice. Both engines mirror VSCode's editing keys (CodeMirror via `utils/editorKeys.ts`: Tab inserts spaces at the cursor, Ctrl+/ comments, Ctrl+Shift+K deletes a line, Alt+Up/Down moves lines, Shift+Alt+Up/Down duplicates, Alt+Z toggles wrap; Monaco gets most of these for free as its own defaults, with Alt+Z/Ctrl+=/Ctrl+- added via `editor.addCommand`). IntelliSense: Monaco's JS/TS is its real built-in language service (actual type inference from JSDoc, not a static list); Java has no built-in language server on either engine, so both use the same curated data (`constants/javaCompletions.ts`, re-adapted for Monaco in `constants/javaCompletionsMonaco.ts`) plus snippet templates (`constants/snippets.ts` for CodeMirror, `constants/monacoSnippets.ts` for Monaco). Both engines share one color palette (`constants/editorTheme.ts`'s `PAPER_EDITOR_PALETTE`, translated to Monaco's theme shape in `constants/monacoTheme.ts`) so switching engines doesn't change colors. The gear button (`components/EditorSettings.tsx`) exposes wrap/font-size toggles (engine-agnostic store in `utils/editorSettingsStore.ts` — each engine subscribes and applies changes through its own API, so switching engines mid-session keeps the same settings) alongside the shortcut list.
